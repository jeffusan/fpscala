...
trait Stream[+A] {
  ...
  def takeWhile(p: A => Boolean): Stream[A] = foldRight(empty[A])((a, b) =>
...
//  def map[B](f: A => B): Stream[B] = foldRight(empty[B]((a, b) => cons(f(a), b)))
  ...
  def startsWith[B](s: Stream[B]): Boolean = unfold(this, s) {
  ...
  def scanRight[B](z: B)(f: (A, => B) => B): Stream[B] = foldRight(Stream.empty[B])((h1, t1) => {
      t1 match {
